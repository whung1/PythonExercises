...
class ComputerPlayer:
    ...
    def get_next_move(self):
        ...
        # Sort possible moves by winning, losing, and unknown (if win or lose) moves
        winning_moves = []
        losing_moves = []
        unknown_moves = []
        ...
        # NOTE: No documentation for choosing when there is non-losing choice, assume random
        if unknown_moves:
            return random.choice(unknown_moves)
        # If the computer thinks it
        ...
        maximal_distance = max(distance for letter, distance in losing_moves)
        maximal_choices = [letter for letter, distance in losing_moves if distance == maximal_distance]
    ...
    def calculate_next_moves(self):
        ...
        sure win (1), unsure (0), or sure loss (-1), along with the max depth

        :return: Dictionary of letters with (letter, win_check, distance) values
    ...
    def __calculate_next_move(self, node, distance):
        ...
        for child in node.get_children():
            cur_state, cur_max_distance = self.__calculate_next_move(child, distance+1)
            max_distance = cur_max_distance if cur_max_distance > max_distance else max_distance
            move_state = cur_state if move_state is None or move_state == cur_state else 0
